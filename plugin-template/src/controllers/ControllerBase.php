 ...
 * role gate — this module is a separate Phalcon module (its own routing
 * namespace under /tags/...), so it can't extend the backend module's
 * class directly, but every controller here is reached only by an
 * already-authenticated backend user (no guest/login pages of its own —
 * those stay in `backend`), so the unauthenticated-controller exemptions
 * that class needs don't apply here. Copied verbatim from
 * application-template's ControllerBase.php (itself copied from
 * xtenstack/requirements-module) — every module built from this template
 * needs this same base class.
 */
